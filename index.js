...
function calculatePrice() {
    ...
    const b = 8000;
    const hectareRanges = [
        { min: 0, max: 50.9, rate1: 1, rate2: 2, no: 2.5 },
    ...
    const totalPriceInWords = toWordsIndianCurrency(Math.round(totalPrice));

    isPriceCalculated = true;
    quotationElement.innerHTML = `Quotation for ${hectaresValue.toFixed(2)} hectares: Rs ${totalPrice.toFixed(2)}<br>(Rupees ${totalPriceInWords})`;
}
// -------------------------------------------------------------------------------------------
...
function Dgps() {
    ...
    let totalPrice = parseFloat(quotationElement.textContent.split('Rs ')[1] || 0);
    ...
    const hectares = hectaresValue.value || 0;
    const totalPriceInWords = toWordsIndianCurrency(Math.round(totalPrice));
